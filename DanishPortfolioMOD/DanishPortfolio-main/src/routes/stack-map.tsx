...
import { stackVisualTheme } from "@/lib/visualThemes";
...
function StackMapPage() {
  ...
  useEffect(() => {
    const hasIncomingFilter = Boolean(q) || (category && category !== "all");
    if (!hasIncomingFilter) return;
  ...
  return (
    <MotionPage
      mood="projects"
      tone={activeCategory === "all" ? "systems" : stackVisualTheme(activeCategory)}
    ...
    >
      ...
      <section className="mt-10 grid gap-5 lg:grid-cols-5">
        ...
        <div className="stack-summary-card rounded-3xl glass p-6 premium-border">
          ...
          <p className="mt-2 text-sm leading-6 text-muted-foreground">
            Writing tied to technical skills, showing how ideas become decisions, systems, and
            lessons that can be applied again.
          </p>
        </div>
      </section>
...
function StackCard({ skill }: { skill: StackRecord }) {
  return (
    <article
      id={`skill-${skill.slug}`}
      data-visual-theme={stackVisualTheme(skill.category)}
      className="semantic-color-card skill-context-card scroll-mt-36 rounded-3xl border border-border/70 bg-background/45 p-5 md:p-6 premium-border"
    >
