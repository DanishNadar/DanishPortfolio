...
import { visualFromPostLike } from "@/lib/visuals";
import portfolio from "@/data/portfolio.json";
import { postVisualTheme } from "@/lib/visualThemes";
  ...
  notFoundComponent: () => (
    <div className="mx-auto max-w-3xl px-6 py-24 text-center">
      ...
      <p className="mt-2 text-muted-foreground">
        That update does not exist yet or has not been published.
      </p>
      <Link to="/posts" className="mt-4 inline-block text-accent underline">
        Back to posts
      </Link>
    </div>
  ),
...
function titleCaseSearchValue(value: string) {
  return value
    ...
    .map((part) => {
      if (
        /^(ai|ml|api|apis|ui|ux|llm|llms|vlm|vlms|dns|spf|dkim|dmarc|aws|can|ros2|rtmaps|sql)$/i.test(
          part,
        )
      )
        return part.toUpperCase();
      if (/^(c\+\+|coqui|xtts-v2|ppo|yolo|opencv|fastapi|streamlit|supabase|vercel)$/i.test(part))
        return part;
...
function stackSearchValue(value: string) {
  ...
  return (
    stackSkillLookup.get(slugKey) ??
    redirects[raw] ??
    redirects[readable] ??
    titleCaseSearchValue(readable)
  );
}
...
function PostTestimonials({
  ...
  const focus =
    relatedStack.slice(0, 3).join(", ") || tags.slice(0, 3).join(", ") || "AI engineering";
  const cards = [
    [
      "Technical signal",
      `${title} turns a portfolio topic into a clear engineering narrative with context, decisions, and practical next-step thinking.`,
    ],
    [
      "Communication signal",
      `The article shows how I explain ${focus} in a way that connects technical depth to business, user, or safety impact.`,
    ],
    [
      "Hiring signal",
      "A prospective employer can use this reflection to understand not just what I built, but how I think, learn, validate, and communicate.",
    ],
  ];
  return (
    <section className="mt-8 grid md:grid-cols-3 gap-4">
      {cards.map(([lens, quote]) => (
        <article key={lens} className="glass premium-border ambient-card rounded-2xl p-5">
          <div className="text-[10px] uppercase tracking-[0.2em] text-accent font-tech">{lens}</div>
...
function PostPage() {
  ...
  const visual = visualFromPostLike({
    cover: post?.cover_image_url,
    suggestedImages: rich?.suggestedImages,
    body,
  });
  const idx = allPosts.findIndex((p) => p.slug === slug);
  const prev = idx > 0 ? allPosts[idx - 1] : null;
  const next = idx >= 0 && idx < allPosts.length - 1 ? allPosts[idx + 1] : null;
  const visualTone = postVisualTheme(type);

  return (
    <MotionPage mood="projects" tone={visualTone} className="mx-auto max-w-7xl px-6 py-16">
      <Link
        to="/posts"
      ...
      <header className="mt-8 glass rounded-[2rem] p-8 md:p-12 bg-gradient-to-br from-blue-500/10 via-rose-800/10 to-slate-900/30 premium-border animated-gradient-surface">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <span className="text-[10px] uppercase tracking-wide px-2 py-0.5 rounded-full glass">
            {status}
          </span>
          {writtenDate && (
            <span className="text-xs text-muted-foreground">
              Written {formatWrittenDate(writtenDate)}
            </span>
          )}
        </div>
        <h1 className="portfolio-title-font post-title-font mt-4 text-4xl md:text-6xl font-bold leading-tight animated-title-glow">
          {title}
        </h1>
        {subtitle && <p className="mt-3 text-xl text-accent">{subtitle}</p>}
        {summary && (
          <p className="mt-5 max-w-5xl font-body text-lg leading-8 text-slate-100/80 md:text-xl md:leading-9">
            {summary}
          </p>
        )}
      </header>
        ...
        <div className="glass rounded-3xl p-7 md:p-10 xl:p-12 readable-panel">
          {body ? (
            ...
            <p className="text-muted-foreground">
              This article is being prepared with project context.
            </p>
          )}
        </div>
        <aside className="space-y-4 xl:sticky xl:top-24">
          <div className="glass ambient-card rounded-2xl p-5">
            ...
            <p className="mt-2 text-xs leading-5 text-muted-foreground">
              Click any item to open the Stack Map already filtered to that exact tool, category, or
              capability.
            </p>
            <div className="mt-3 flex flex-wrap gap-2">
              {relatedStack.length ? (
                relatedStack.map((s) => <StackMapPill key={s} value={s} />)
              ) : (
                <span className="text-xs text-muted-foreground">
                  Related stack links will appear here when this article has stack context.
                </span>
              )}
            </div>
          </div>
          <div className="glass ambient-card rounded-2xl p-5">
            ...
            <p className="mt-2 text-xs leading-5 text-muted-foreground">
              Tags also act as quick filters so visitors can keep exploring the portfolio by topic.
            </p>
            <div className="mt-3 flex flex-wrap gap-2">
              {tags.map((t) => (
                <StackMapPill key={t} value={`#${t}`} />
              ))}
            </div>
          </div>
        </aside>
      ...
      {post?.why_this_matters && (
        <div className="mt-8 glass rounded-2xl p-6 border-l-2 border-accent">
          <div className="text-xs uppercase tracking-widest text-accent">Why this matters</div>
          <p className="mt-2 text-foreground/90">{post.why_this_matters}</p>
        </div>
      )}

      <section className="mt-10">
        <div className="text-xs uppercase tracking-widest text-accent mb-3">
          Testimonial-style readout
        </div>
      ...
      {relatedProjects.length > 0 && (
        <section className="mt-10">
          <h2 className="text-xs uppercase tracking-widest text-accent mb-3">Related projects</h2>
          <div className="grid md:grid-cols-2 gap-3">
            {relatedProjects.map((p) => (
              <Link
                ...
                to="/projects/$slug"
                params={{ slug: p.slug }}
                className="glass ambient-card rounded-xl p-5 hover:glow-blue transition"
              >
                <div className="portfolio-title-font text-sm font-semibold">{p.title}</div>
      ...
      <div className="mt-10 flex flex-wrap gap-3">
        {post?.source_url && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-2 glass px-4 py-2 rounded-lg text-sm font-semibold hover:glow-blue transition"
          >
        ...
        {sourceLinks.map((source) => (
          <a
            ...
            className="inline-flex items-center gap-2 glass px-4 py-2 rounded-lg text-sm font-semibold hover:glow-blue transition"
          >
            <ExternalLink className="h-3.5 w-3.5" /> {source.label}
          </a>
        ))}
        <Link
          to="/contact"
          className="inline-flex items-center gap-2 bg-gradient-rb text-background px-4 py-2 rounded-lg text-sm font-semibold"
        >
          Contact / discuss this
        </Link>
        <Link
          to="/stack-map"
          search={{ page: 1, q: relatedStack[0] ?? "", category: "all" }}
          className="inline-flex items-center gap-2 glass px-4 py-2 rounded-lg text-sm font-semibold"
        >
          <Newspaper className="h-3.5 w-3.5" /> Related stack
        </Link>
      </div>

      {(prev || next) && (
        <div className="mt-16 grid grid-cols-2 gap-3 border-t border-border pt-6">
          <div>
            {prev && (
              <Link
                to="/posts/$slug"
                params={{ slug: prev.slug }}
                className="block glass rounded-xl p-4 hover:glow-blue transition"
              >
                <div className="text-[10px] uppercase tracking-wide text-muted-foreground">
                  ← Previous
                </div>
                <div className="portfolio-title-font text-sm font-semibold mt-1 line-clamp-2">
                  {prev.title}
                </div>
              </Link>
            )}
