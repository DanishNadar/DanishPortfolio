    ...
    >
      ...
      <section className="section-atmosphere section-atmosphere-community mt-6 grid gap-8 rounded-[2rem] p-6 md:p-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
        <ImageSlot
          ...
          src={friend.image}
          fallbackSrc={friend.placeholderImage}
          aspect="aspect-[4/3]"
          imgPosition={friend.portraitCrop?.position}
          imgScale={friend.portraitCrop?.scale}
          imgOrigin={friend.portraitCrop?.origin}
        />
        <div>
          ...
          <div className="mt-7">
            {friend.linkedinUrl ? (
              ...
              <Link
                to="/contact"
                className="inline-flex items-center gap-2 rounded-xl border border-border/70 bg-background/35 px-4 py-3 text-sm text-muted-foreground hover:border-accent hover:text-accent"
              >
                <Linkedin className="h-4 w-4 text-accent" />
                Connect through Danish
              </Link>
            )}
          </div>
        </div>
      ...
      <section className="mt-14">
        ...
        <p className="mt-4 max-w-5xl text-base leading-8 text-muted-foreground">
          {friend.sharedWorkSummary}
        </p>
        {projects.length ? (
      ...
      <section className="mt-14 grid gap-5 md:grid-cols-2">
        {friend.gallery.map((image, index) => (
          <ImageSlot
            ...
            src={image}
            fallbackSrc={friend.galleryPlaceholders[index]}
            aspect="aspect-[4/3]"
          />
        ))}
      </section>
      ...
      <section className="mt-14 glass premium-border rounded-[2rem] p-7 md:p-10">
        ...
        <h2 className="mt-3 text-3xl font-display font-bold md:text-4xl">
          Thank you, {friend.gratitudeName ?? friend.name.split(" ")[0]}.
        </h2>
        <div className="mt-6 grid gap-5 lg:grid-cols-2">
          <article className="rounded-3xl border border-accent/25 bg-background/35 p-6">
            ...
            <p className="mt-4 text-base leading-8 text-foreground/85">{friend.gratitude}</p>
          </article>
          <article className="rounded-3xl border border-accent/25 bg-background/35 p-6">
            <div className="text-[10px] uppercase tracking-[0.18em] text-accent font-tech">
              Looking ahead
            </div>
            <p className="mt-4 text-base leading-8 text-foreground/85">{friend.legacy}</p>
          </article>
        </div>
      </section>
