...
function ProjectsIndex() {
  ...
  return (
    <MotionPage className="projects-page mx-auto max-w-[100rem] px-6 lg:px-12 py-16">
      ...
      <p className="mt-3 text-muted-foreground max-w-2xl">{projects.length} case studies across AI, robotics, autonomy, security, and product.</p>
          ...
          <motion.div key={p.id} layout initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}>
            <Link to="/projects/$slug" params={{ slug: p.slug }} className="group block glass premium-border ambient-card rounded-[1.85rem] p-0 h-full min-h-[25rem] hover:glow-blue transition hover:-translate-y-1 overflow-hidden">
              <div className="aspect-video bg-muted/20 overflow-hidden">
                <img src={detail?.gallery?.[0]?.src ?? (p.cover_image ? `/assets/projects/${p.cover_image}` : "/portfolio_images/stackmap/workstation-stack.jpg")} alt={`${detail?.title ?? p.title} visual`} className="h-full w-full object-cover transition duration-300 group-hover:scale-[1.03]" />
              </div>
              ...
              <p className="mt-2 text-sm text-muted-foreground line-clamp-3">{detail?.heroStatement ?? p.summary}</p>
