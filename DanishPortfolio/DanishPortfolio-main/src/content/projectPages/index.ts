...
export const projectPages = [observ_e, selvam_valuations, ecocar_sensor_fusion, a_little_tech_for_you, rl_autonomous_driving, lane_detection_salad, dns_security_scanner, ttp_outreach_automation, scammantha, jtr_agent, aila_avatar, fraud_detection, course_recommendation, ai_headshot_platform, phishing_detector, spiron_assistant, shopping_bot];
export const projectPageBySlug: Record<string, (typeof projectPages)[number]> = Object.fromEntries(projectPages.map((p) => [p.slug, p]));
export function getProjectPage(slug: string) { return projectPageBySlug[slug] ?? null; }
export * from "./types";
