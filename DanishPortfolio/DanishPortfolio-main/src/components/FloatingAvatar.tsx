...
import { getAvatarBridgeStatus, requestAvatarSpeech, type AvatarBridgeStatus } from "@/lib/liveAvatarClient";
  ...
  return (
    <div className={`dn-ai-orb ${speaking ? "dn-ai-orb-speaking" : ""}`} aria-hidden="true">
      ...
      <div className="dn-ai-orb-core">
        <img src="/brand/dn-logo.png" alt="" className="h-full w-full object-contain dn-response-logo-img" draggable={false} loading="eager" />
      </div>
    </div>
  );
});
...
const INITIAL_MESSAGES: Msg[] = [
  { role: "avatar", text: "Ask me anything — I'll be most useful when the question connects to Danish, his work, or this portfolio." },
];
...
function AvatarMediaView({ media, className = "" }: { media: AvatarMedia; className?: string }) {
  if (media.kind === "video") {
    return <video key={media.src} src={media.src} className={className} autoPlay muted playsInline loop />;
  }

  return <img src={media.src} alt="Danish avatar" className={className} loading="eager" draggable={false} />;
}

function ScreenAvatarStage({ media, open, speaking }: { media: AvatarMedia; open: boolean; speaking: boolean }) {
  return (
    <div
...
export function FloatingAvatar() {
  ...
  const speakReply = useCallback((text: string, requestId: number) => {
    ...
    utterance.voice =
      voices.find((voice) => /natural|neural|online|google|microsoft/i.test(voice.name) && /^en/i.test(voice.lang)) ??
  ...
  const playGeneratedSpeech = useCallback(async (audioUrl: string, requestId: number) => {
    if (audioRef.current) {
      audioRef.current.pause();
      audioRef.current = null;
    }
  ...
  const send = useCallback(async (text: string) => {
    ...
    setMsgs((current) => {
      const next = [...current, { role: "user", text: cleanText } satisfies Msg].slice(-MAX_VISIBLE_MESSAGES);
    ...
    try {
      ...
      requestAvatarSpeech(reply).then((result) => {
        ...
        if (result.videoUrl) {
          setAvatarMedia({ kind: "video", src: `${result.videoUrl}?t=${Date.now()}`, status: "rendered" });
        }
        if (result.imageUrl) {
        ...
        if (result.audioUrl) {
          speechHandled = true;
          playGeneratedSpeech(`${result.audioUrl}?t=${Date.now()}`, requestId).catch(() => speakReply(reply, requestId));
          return;
        }
        speechHandled = true;
        speakReply(reply, requestId);
      ...
      if (requestIdRef.current !== requestId) return;
      const reply = "I could not reach the DN engine right now, but the portfolio still has the main paths: Projects, Autonomy, Stack Map, Posts, About, and Contact.";
      setMsgs((current) => {
        const next = [...current, { role: "avatar", text: reply, suggestedRoute: "/projects", suggestedLabel: "Explore projects" } satisfies Msg].slice(-MAX_VISIBLE_MESSAGES);
        msgsRef.current = next;
        return next;
      });
  ...
  return (
    <>
      ...
      <ScreenAvatarStage media={avatarMedia} open={open} speaking={speaking} />
          ...
          >
            ...
            <div className="dn-response-header flex items-center justify-between p-3 border-b border-border">
              <div className="flex items-center gap-2">
                <div className={`dn-live-avatar ${speaking ? "dn-live-avatar-speaking" : ""}`} data-status={avatarMedia.status}>
            ...
            <div ref={scrollRef} className="dn-response-messages overflow-auto p-4 space-y-3">
              {msgs.map((m, i) => (
                <div key={`${m.role}-${i}`} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                  <div className={`dn-response-bubble max-w-[86%] px-4 py-3 rounded-2xl text-[0.95rem] leading-relaxed ${m.role === "user" ? "dn-response-bubble-user" : "dn-response-bubble-avatar"}`}>
                    {m.text}
                    {m.suggestedRoute && (
                      <Link to={m.suggestedRoute} onClick={closePanel} className="mt-2 block text-xs font-semibold text-accent hover:underline">
            ...
            <div className="dn-response-footer p-3 border-t border-border">
              {QUICK_PROMPTS.length > 0 && (
                <div className="mb-2 flex flex-wrap gap-1.5">
                  {QUICK_PROMPTS.map((q) => (
                    <button key={q} type="button" onClick={() => send(q)} className="text-xs px-2.5 py-1.5 rounded-full border border-border text-accent hover:bg-muted/30">
                      {q}
                    </button>
                  ))}
                </div>
            ...
            <div id="dn-response-input-hint" className="dn-response-caption px-3 pb-3 text-[10px] text-muted-foreground">
