...
export const postPages = [
  linux_first_ai_engineering_workflow,
  intelligent_systems_for_safer_roads,
  starkhacks_2026_observ_e_win,
  ecocar_sensor_fusion_reflection,
  building_an_rl_lane_keeping_simulator,
  lane_detection_backbone_bake_off,
  automating_spf_dkim_dmarc_screening,
  ttp_streamlit_outreach_automation,
  scammantha_scam_awareness_game,
  career_bootcamp_takeaways,
  networking_event_ink_factory,
  building_an_ai_avatar_portfolio,
  why_accessibility_robotics,
  debugging_vercel_supabase_deploys,
  ...generatedStackPosts,
];
export const postPageBySlug: Record<string, (typeof postPages)[number]> = Object.fromEntries(
  postPages.map((p) => [p.slug, p]),
);
export function getPostPage(slug: string) {
  const post = postPageBySlug[slug];
  if (!post) return null;
  return {
    ...post,
    summary: post.summary
      .replace(/supports my portfolio as/gi, "supports my engineering work as")
      .replace(/portfolio systems/gi, "production systems"),
    body: post.body
      .replace(
        /In my portfolio, it is not listed as decoration; it shows up because/gi,
        "It matters because",
      )
      .replace(
        /A strong portfolio should not read like a keyword list\. It should show how each capability helps build something useful\./gi,
        "The important point is how each capability contributes to a useful, working system.",
      )
      .replace(/supports my portfolio as/gi, "supports my engineering work as"),
  };
}
